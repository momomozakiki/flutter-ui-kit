...
- **atoms** — `lib/src/atoms/`: primitive widgets wrapping Material controls with the kit's consistent look (`UiButton`, `UiDropdown`, `UiTextField`, etc.).
- **compositions** — `lib/src/molecules/` (stateless) or `lib/src/organisms/` (local UI state): generic groupings of atoms (`UiResponsive` is a molecule; `UiTuningPanel`, `UiUnderMaintenance` are organisms).
...
- [`lib/src/atoms/ui_button.dart`](../../lib/src/atoms/ui_button.dart) (Interface Segregation and semantic color example).
